...
- **管线类型预设（P4）**：10 类管线（天然气长输/原油/集输/注水/海底/城市燃气/化工/酸性气田/输水/掺氢），一键"套用典型工况"联动主导威胁与推荐管材
- **后果分析与维护建议（P4）**：依据管线类型/管径/压力/位置/壁损评估泄漏后果等级，并针对 CO₂内腐蚀/H₂S开裂/外部腐蚀/SCC/MIC/冲蚀/电偶腐蚀 7 类威胁给出立即措施·工程修复·监测·标准依据
...
- 新增**文献与公开数据覆盖**（`research_references.md`：PHMSA/PRCI/EGIG/CER/CEPA/EPRG/DNV/MatWeb/DECHEMA/NIMS/corrdata 等数据库链接 + 关键论文与标准引用 + NDT/完整性管理标准 + 管线类型预设）
- Dify Cloud RAG 流式响应 + LRU 缓存；本地模式自动收录 `data/standards/` 下全部 `.md`

### Tab 3: 数据探索
- 数据分布分析、相关性热力图、特征重要性、多模型对比（7+ 种：GBR / RF / DT / LR / MLP / SVR / XGBoost / 投票集成）
...
### Tab 9: 无损检测 NDT（P4 新增）
- **NDT 方法知识库**：11 种无损检测方法（MFL / UT-CD / EMAT / Caliper / PAUT / RT / MT / PT / ET / 导波 / AE）的原理、可检缺陷、灵敏度、局限、典型用途与标准
- **ILI 工具选型**：依据主导威胁与管线可检性推荐 MFL / UT-CD 组合 run，并给出 ECDA / ICDA / SCCDA 直接评估路径
- **API 1163 性能量化**：POD（检出概率）/ POI（识别概率）/ 尺寸精度 概念与"组合 run"最佳实践（避免单一工具盲区）
